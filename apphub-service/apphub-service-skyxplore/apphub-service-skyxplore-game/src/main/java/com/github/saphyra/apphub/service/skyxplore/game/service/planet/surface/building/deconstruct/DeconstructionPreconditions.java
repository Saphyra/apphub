...
class DeconstructionPreconditions {
    ...
    void checkIfBuildingCanBeDeconstructed(GameData gameData, Building building) {
        ...
        if (isNull(buildingData)) {
            log.info("{} is not StorageBuilding, no need to check storage capacity.", building.getDataId());
            return;
        }
        ...
        int storedAmount = storedResourceAmountQueryService.getActualStorageAmount(gameData, building.getLocation(), buildingData.getStores());
