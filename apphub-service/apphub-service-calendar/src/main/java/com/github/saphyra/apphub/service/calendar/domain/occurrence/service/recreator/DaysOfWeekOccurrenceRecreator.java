...
@Slf4j
//TODO unit test
class DaysOfWeekOccurrenceRecreator implements OccurrenceRecreator {
    ...
    public void recreateOccurrences(UpdateEventContext context) {
        ...
        Set<DayOfWeek> daysOfWeek = objectMapperWrapper.convertValue(event.getRepetitionData(), typeReference);

        LocalDate date = dateTimeUtil.earlier(event.getStartDate(), dateTimeUtil.earliest(existingOccurrences.keySet()));
        LocalDate endDate = dateTimeUtil.later(event.getStartDate(), dateTimeUtil.latest(existingOccurrences.keySet()));

        while (!date.isAfter(endDate)) {
            boolean shouldHaveOccurrence = daysOfWeek.contains(date.getDayOfWeek());
