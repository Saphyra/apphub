...
@Slf4j
//TODO unit test
class TimeColumnDataService implements ColumnDataService {
    ...
    public boolean canProcess(ColumnType columnType) {
        return columnType == ColumnType.TIME;
    }
    ...
    public Optional<TableFileUploadResponse> save(UUID userId, UUID listItemId, UUID rowId, TableColumnModel model) {
        ...
        ColumnTypeDto columnTypeDto = columnTypeFactory.create(column.getDimensionId(), userId, ColumnType.TIME);
    ...
    public void clone(ListItem clone, UUID rowId, Dimension originalColumn) {
        ...
        ColumnTypeDto columnTypeDto = columnTypeFactory.create(clonedColumn.getDimensionId(), clone.getUserId(), ColumnType.TIME);
        columnTypeDao.save(columnTypeDto);
