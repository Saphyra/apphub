...
class EditTableRowService {
    ...
    List<TableFileUploadResponse> editTableRows(ListItem listItem, List<TableRowModel> rows) {
        deleteRows(listItem.getListItemId(), rows);
        return updateRows(listItem, rows);
    }

    private void deleteRows(UUID listItemId, List<TableRowModel> rows) {
    ...
    private List<TableFileUploadResponse> updateRows(ListItem listItem, List<TableRowModel> rows) {
        return rows.stream()
