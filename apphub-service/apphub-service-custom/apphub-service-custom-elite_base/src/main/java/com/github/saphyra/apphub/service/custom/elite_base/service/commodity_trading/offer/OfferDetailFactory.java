...
class OfferDetailFactory {
    ...
    List<OfferDetail> create(List<Offer> offers) {
        FutureWrapper<Map<UUID, ItemLocationData>> locationDataMap = executorServiceBean.asyncProcess(() -> loadStations(offers));
        FutureWrapper<Map<UUID, StarSystemData>> starSystemDataMap = executorServiceBean.asyncProcess(() -> loadStarSystemData(offers));
        FutureWrapper<Map<UUID, List<Power>>> powers = executorServiceBean.asyncProcess(() -> loadPowers(offers));
        Map<UUID, Body> bodies = loadBodies(locationDataMap.get().getOrThrow().values().stream().map(ItemLocationData::getBodyId).filter(Objects::nonNull).toList());
