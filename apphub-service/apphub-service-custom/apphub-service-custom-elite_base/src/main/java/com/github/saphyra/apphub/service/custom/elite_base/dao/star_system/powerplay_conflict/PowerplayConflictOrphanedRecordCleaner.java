...
@Component
@Slf4j
class PowerplayConflictOrphanedRecordCleaner extends OrphanedRecordCleaner {
    private final JdbcTemplate jdbcTemplate;

    PowerplayConflictOrphanedRecordCleaner(ErrorReporterService errorReporterService, JdbcTemplate jdbcTemplate) {
    ...
    protected int doCleanup() {
        String sql = SqlBuilder.delete()
            ...
            .condition(new NotExistsCondition(SqlBuilder.select()
                ...
                .condition(new Equation(new QualifiedColumn(TABLE_STAR_SYSTEM_DATA, COLUMN_STAR_SYSTEM_ID), new QualifiedColumn(TABLE_POWERPLAY_CONFLICT, COLUMN_STAR_SYSTEM_ID)))
            ))
            .build();
