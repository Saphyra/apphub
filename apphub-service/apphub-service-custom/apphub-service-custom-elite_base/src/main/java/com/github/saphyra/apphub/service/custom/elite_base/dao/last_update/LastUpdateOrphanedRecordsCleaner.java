...
@Component
@Slf4j
class LastUpdateOrphanedRecordsCleaner extends OrphanedRecordCleaner {
    private final JdbcTemplate jdbcTemplate;

    LastUpdateOrphanedRecordsCleaner(ErrorReporterService errorReporterService, JdbcTemplate jdbcTemplate) {
    ...
    protected int doCleanup() {
        String sql = SqlBuilder.delete()
            ...
            .condition(new NotExistsCondition(SqlBuilder.select()
                ...
                .condition(new Equation(new QualifiedColumn(TABLE_STATION, COLUMN_ID), new QualifiedColumn(TABLE_LAST_UPDATE, COLUMN_EXTERNAL_REFERENCE)))
            ))
            .and()
            .condition(new NotExistsCondition(SqlBuilder.select()
                .column("1")
                .from(new QualifiedTable(SCHEMA, TABLE_FLEET_CARRIER))
                .condition(new Equation(new QualifiedColumn(TABLE_FLEET_CARRIER, COLUMN_ID), new QualifiedColumn(TABLE_LAST_UPDATE, COLUMN_EXTERNAL_REFERENCE)))
            ))
            .build();
