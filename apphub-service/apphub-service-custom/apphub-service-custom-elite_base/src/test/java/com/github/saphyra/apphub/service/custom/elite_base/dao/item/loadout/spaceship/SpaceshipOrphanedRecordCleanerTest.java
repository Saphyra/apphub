...
@ActiveProfiles("test")
class SpaceshipOrphanedRecordCleanerTest {
    ...
    @Autowired
    private SpaceshipOrphanedRecordCleaner underTest;
    ...
    @Test
    void getOrphanage() {
        assertThat(underTest.getOrphanage()).isEqualTo(Orphanage.ITEM_SPACESHIP);
    }
    ...
    void cleanup() {
        ...
        Spaceship stationSpaceship = Spaceship.builder()
            .externalReference(STATION_ID)
            .itemName(ITEM_NAME)
            .marketId(MARKET_ID)
            .build();
        SpaceshipDao.save(stationSpaceship);
        Spaceship fleetCarrierSpaceship = Spaceship.builder()
            .externalReference(FLEET_CARRIER_ID)
            .itemName(ITEM_NAME)
            .marketId(MARKET_ID)
            .build();
        SpaceshipDao.save(fleetCarrierSpaceship);
        Spaceship orphanedSpaceship = Spaceship.builder()
            .externalReference(UUID.randomUUID())
            .itemName(ITEM_NAME)
            .marketId(MARKET_ID)
            .build();
        SpaceshipDao.save(orphanedSpaceship);

        bufferSynchronizationService.synchronizeAll();

        assertThat(underTest.cleanupOrphanedRecords()).isEqualTo(1);

        assertThat(SpaceshipDao.findAll()).containsExactlyInAnyOrder(stationSpaceship, fleetCarrierSpaceship);
    }
}