...
@ActiveProfiles("test")
class EquipmentOrphanedRecordCleanerTest {
    ...
    @Autowired
    private EquipmentOrphanedRecordCleaner underTest;
    ...
    @Test
    void getOrphanage() {
        assertThat(underTest.getOrphanage()).isEqualTo(Orphanage.ITEM_EQUIPMENT);
    }
    ...
    void cleanup() {
        ...
        Equipment stationEquipment = Equipment.builder()
            .externalReference(STATION_ID)
            .itemName(ITEM_NAME)
            .marketId(MARKET_ID)
            .build();
        equipmentDao.save(stationEquipment);
        Equipment fleetCarrierEquipment = Equipment.builder()
            .externalReference(FLEET_CARRIER_ID)
            .itemName(ITEM_NAME)
            .marketId(MARKET_ID)
            .build();
        equipmentDao.save(fleetCarrierEquipment);
        Equipment orphanedEquipment = Equipment.builder()
            .externalReference(UUID.randomUUID())
            .itemName(ITEM_NAME)
            .marketId(MARKET_ID)
            .build();
        equipmentDao.save(orphanedEquipment);

        bufferSynchronizationService.synchronizeAll();

        assertThat(underTest.cleanupOrphanedRecords()).isEqualTo(1);

        assertThat(equipmentDao.findAll()).containsExactlyInAnyOrder(stationEquipment, fleetCarrierEquipment);
    }
}