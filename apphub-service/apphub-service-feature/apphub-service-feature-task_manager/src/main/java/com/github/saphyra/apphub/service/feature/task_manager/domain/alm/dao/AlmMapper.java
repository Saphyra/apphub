...
class AlmMapper extends ConverterBase<Map<String, AttributeValue>, Alm> {
    ...
    protected Map<String, AttributeValue> processDomainConversion(Alm domain) {
        ...
        result.put(COLUMN_PK, AttributeValue.builder().s(domain.getPrincipalType() + "#" + domain.getPrincipal()).build());
        result.put(COLUMN_SK, AttributeValue.builder().s(domain.getObjectType() + "#" + domain.getObjectId()).build());
    ...
    protected Alm processEntityConversion(Map<String, AttributeValue> entity) {
        String[] pkParts = entity.get(COLUMN_PK).s().split("#", 2);
