...
import static com.github.saphyra.apphub.service.feature.elite_base.common.DatabaseConstants.COLUMN_COMMODITY_NAME;
...
@Slf4j
@Profile("!test")
class CommodityAveragePriceLastUpdateMigrator {
    ...
    @PostConstruct
    void migrate() {
        log.info("Migrating CommodityAveragePrice lastUpdates");
        ...
        log.info("{} CommodityAveragePrice migrated", totalCount);
    }

    private void clearLastUpdate(List<BiWrapper<String, String>> batch) {
        List<String> sqls = batch.stream()
            .map(entry -> SqlBuilder.update(new QualifiedTable(SCHEMA, TABLE_COMMODITY_AVERAGE_PRICE))
                .set(new DefaultColumn(COLUMN_LAST_UPDATE), new NullValue())
                .condition(new Equation(new DefaultColumn(COLUMN_COMMODITY_NAME), new WrappedValue(entry.getEntity1())))
                .build())
            .toList();
        ...
        executorServiceBean.processCollectionWithWait(
            ...
            record -> {
                Map<String, String> parameters = Map.of(
                    ...
                    COLUMN_OBJECT_TYPE, ObjectType.COMMODITY_AVERAGE_PRICE.name(),
    ...
    private List<BiWrapper<String, String>> fetchBatch() {
        String sql = SqlBuilder.select()
            .columns(COLUMN_COMMODITY_NAME, COLUMN_LAST_UPDATE)
            .from(new QualifiedTable(SCHEMA, TABLE_COMMODITY_AVERAGE_PRICE))
            ...
            rs -> {
                ...
                while (rs.next()) {
                    result.add(new BiWrapper<>(
                        rs.getString(COLUMN_COMMODITY_NAME),
