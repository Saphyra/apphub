...
@Component
@Slf4j
class MinorFactionOrphanedRecordCleaner extends BatchOrphanedRecordCleaner {
    private final JdbcTemplate jdbcTemplate;
    private final EliteBaseProperties eliteBaseProperties;

    MinorFactionOrphanedRecordCleaner(ErrorReporterService errorReporterService, MonitoringInstruments monitoringInstruments, JdbcTemplate jdbcTemplate, EliteBaseProperties eliteBaseProperties) {
    ...
    protected List<String> fetchIds() {
        String sql = SqlBuilder.select()
            ...
            .except(SqlBuilder.select().column(COLUMN_MINOR_FACTION_ID).from(new QualifiedTable(SCHEMA, TABLE_STAR_SYSTEM_MINOR_FACTION_MAPPING)))
        ...
        return jdbcTemplate.query(sql, rs -> {
            ...
            while (rs.next()) {
                result.add(rs.getString(COLUMN_ID));
            }
            return result;
        });
    }
    ...
    protected void delete(List<String> idsToDelete) {
        String sql = SqlBuilder.delete()
            .from(new QualifiedTable(SCHEMA, TABLE_MINOR_FACTION))
            .condition(new InCondition(new DefaultColumn(COLUMN_ID), new ListValue(idsToDelete)))
            .build();
