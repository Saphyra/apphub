...
import com.github.saphyra.apphub.service.feature.skyxplore.data.save_game.dao.GameItemService;
...
class SaveGameItemService {
    ...
    SaveGameItemService(List<GameItemService> savers, ObjectMapper objectMapper, ErrorReporterService errorReporterService) {
    ...
    void save(List<Object> items) {
        log.info("Saving {} number of gameItems for game {}...", items.size(), objectMapper.convertValue(items.get(0), GameItem.class).getGameId());
        items.stream()
            .map(o -> new BiWrapper<>(o, objectMapper.convertValue(o, GameItem.class).getType()))
    ...
    private void save(GameItemType gameItemType, List<Object> gameItemObjects) {
        try {
            ...
            List<GameItem> models = gameItemObjects.stream()
                .map(o -> objectMapper.convertValue(o, gameItemType.getModelType()))
