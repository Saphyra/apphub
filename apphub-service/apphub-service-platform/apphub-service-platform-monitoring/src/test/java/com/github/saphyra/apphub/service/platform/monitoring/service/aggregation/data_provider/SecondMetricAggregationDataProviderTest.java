...
class SecondMetricAggregationDataProviderTest {
    ...
    @InjectMocks
    private SecondMetricAggregationDataProvider underTest;
    ...
    @Test
    void getType() {
        assertThat(underTest.getType()).isEqualTo(MetricDataType.SECOND);
    }

    @Test
    void getExpirationTime() {
        given(monitoringProperties.getAggregation()).willReturn(Map.of(MetricDataType.SECOND, aggregation));
        given(aggregation.getExpirationDuration()).willReturn(EXPIRATION_DURATION);
        given(dateTimeUtil.getCurrentDateTime()).willReturn(CURRENT_TIME);


        assertThat(underTest.getExpirationTime()).isEqualTo(CURRENT_TIME.withNano(0).withSecond(0).minus(EXPIRATION_DURATION));
    }
    ...
    @Test
    void getResultType() {
        assertThat(underTest.getResultType()).isEqualTo(MetricDataType.MINUTE);
    }
}