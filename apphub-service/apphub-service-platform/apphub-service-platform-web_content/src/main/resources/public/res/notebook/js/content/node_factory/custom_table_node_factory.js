function customTableNodeFactory(parent, itemDetails, displayOpenParentCategoryButton){
    const node = document.createElement("DIV");
        ...
        node.onclick = function(){
            customTableViewController.viewCustomTable(itemDetails.id);
        }
    ...
    node.appendChild(actionButtonFactory.create(
        ...
        itemDetails,
        node,
        function(){deleteCustomTable(itemDetails.id, itemDetails.title)},
        displayOpenParentCategoryButton
    ));
    return node;

    function deleteCustomTable(listItemId, title){
