...
public class DeleteLabelTest extends BackEndTest {
    ...
    public void deleteLabel() {
        ...
        ShareObjectRequest shareLabelRequest = ShareObjectRequest.builder()
            ...
            .grants(Set.of(Grant.DELETE, Grant.VIEW))
            .build();
        CalendarShareActions.shareObject(getServerPort(), ownerToken, shareLabelRequest);

        //Delete label
        CalendarLabelActions.deleteLabel(getServerPort(), sharedWithToken, labelId);
    ...
    @Test(groups = {"be", "calendar"})
    public void deleteLabel_noGrant() {
        RegistrationParameters ownerData = RegistrationParameters.validParameters();
        String ownerToken = IndexPageActions.registerAndLogin(getServerPort(), ownerData);
        UUID ownerId = UserDynamoDbRepository.getUserIdByEmail(ownerData.getEmail());
        ...
        //Delete label
        ResponseValidator.verifyNotFound(CalendarLabelActions.getDeleteLabelResponse(getServerPort(), sharedWithToken, labelId));

        //Verify label is deleted
        assertThat(CalendarLabelActions.getLabels(getServerPort(), sharedWithToken)).hasSize(1);
        assertThat(CalendarLabelActions.getLabels(getServerPort(), ownerToken)).hasSize(1);
    }
}
