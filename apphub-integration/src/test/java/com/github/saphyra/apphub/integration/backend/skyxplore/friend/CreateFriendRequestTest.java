...
public class CreateFriendRequestTest extends BackEndTest {
    ...
    public void createFriendRequest() {
        ...
        characterNotFound(accessToken1);
        createFriendRequest(accessToken1, accessToken2, model1, model2, userId2);
        friendRequestAlreadyExists(accessToken1, userId2);
        friendshipAlreadyExists(accessToken1, accessToken2, userId2);
    }

    private static void characterNotFound(String accessToken1) {
        Response characterNotFoundResponse = SkyXploreFriendActions.getCreateFriendRequestResponse(getServerPort(), accessToken1, UUID.randomUUID());
        verifyErrorResponse(characterNotFoundResponse, 404, ErrorCode.USER_NOT_FOUND);
    }

    private static void createFriendRequest(String accessToken1, String accessToken2, SkyXploreCharacterModel model1, SkyXploreCharacterModel model2, UUID userId2) {
        ApphubWsClient friendClient = ApphubWsClient.createSkyXploreMainMenu(getServerPort(), accessToken2, accessToken2);

        SentFriendRequestResponse sentFriendRequestResponse = SkyXploreFriendActions.createFriendRequest(getServerPort(), accessToken1, userId2);
        ...
        assertThat(sentFriendRequests.getFirst().getFriendName()).isEqualTo(model2.getName());
        ...
        assertThat(incomingFriendRequests.getFirst().getSenderName()).isEqualTo(model1.getName());
    ...
    private static void friendRequestAlreadyExists(String accessToken1, UUID userId2) {
        Response friendRequestAlreadyExistsResponse = SkyXploreFriendActions.getCreateFriendRequestResponse(getServerPort(), accessToken1, userId2);
        verifyErrorResponse(friendRequestAlreadyExistsResponse, 409, ErrorCode.FRIEND_REQUEST_ALREADY_EXISTS);
    }

    private static void friendshipAlreadyExists(String accessToken1, String accessToken2, UUID userId2) {
        ...
        Response friendshipAlreadyExistsResponse = SkyXploreFriendActions.getCreateFriendRequestResponse(getServerPort(), accessToken1, userId2);
