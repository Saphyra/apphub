...
public class CreateOccurrenceTest extends SeleniumTest {
    ...
    public void createOccurrenceToSharedEvent() {
        ...
        // Create occurrence
        LocalDate occurrenceDate = parameters.getStartDate().plusMonths(1);
        ...
        CalendarEventPageActions.setCreateOccurrenceDate(sharedWithDriver, occurrenceDate);
        CalendarEventPageActions.createOccurrence(sharedWithDriver);
        ToastMessageUtil.verifySuccessToast(sharedWithDriver, LocalizedText.CALENDAR_OCCURRENCE_CREATED);
        CalendarEventPageActions.backFromEdit(sharedWithDriver);

        // Verify occurrence is created.
        AwaitilityWrapper.retry(() -> CalendarIndexPageActions.setReferenceDate(sharedWithDriver, occurrenceDate));
        AwaitilityWrapper.awaitAssert(() -> assertThat(CalendarIndexPageActions.getOccurrencesOnDate(sharedWithDriver, occurrenceDate))
            .singleElement()
            .returns(parameters.getTitle(), CalendarOccurrence::getTitle));

        AwaitilityWrapper.retry(() -> CalendarIndexPageActions.setReferenceDate(ownerDriver, occurrenceDate));
        AwaitilityWrapper.awaitAssert(() -> assertThat(CalendarIndexPageActions.getOccurrencesOnDate(ownerDriver, occurrenceDate))
            .singleElement()
            .returns(parameters.getTitle() + Constants.SHARED_SUFFIX, CalendarOccurrence::getTitle));
    }
}
