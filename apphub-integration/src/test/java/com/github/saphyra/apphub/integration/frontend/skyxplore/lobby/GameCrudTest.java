...
public class GameCrudTest extends SeleniumTest {
    ...
    public void gameCrud() {
        ...
        startGame_notAllMembersReady(driver1, driver2);
    ...
    private static void startGame(WebDriver driver1, WebDriver driver2, RegistrationParameters userData1, RegistrationParameters userData2) {
        ...
        SkyXploreLobbyPageActions.setReady(driver1);
        SkyXploreLobbyPageActions.setReady(driver2);

        LobbyPlayer lobbyPlayer = SkyXploreLobbyPageActions.findPlayerValidated(driver1, userData1.getUsername());
        AwaitilityWrapper.createDefault()
    ...
    private static void startGame_notAllMembersReady(WebDriver driver1, WebDriver driver2) {
        AwaitilityWrapper.createDefault()
            .until(() -> SkyXploreLobbyPageActions.getOnlineFriends(driver1).isEmpty())
            .assertTrue("Friend is still marked as active despite accepting invitation.");

        AwaitilityWrapper.createDefault()
            .until(() -> SkyXploreLobbyPageActions.getOnlineFriends(driver2).isEmpty())
            .assertTrue("Friend is still marked as active despite accepting invitation.");
    ...
    private static void checkIfNotMemberFriendIsNotAvailableToInvite(WebDriver driver1) {
        AwaitilityWrapper.awaitAssert(() -> assertThat(SkyXploreLobbyPageActions.getOnlineFriends(driver1)).isEmpty());
    }
    ...
    private static void startGame_notAllMembersPresent(WebDriver driver1) {
        ...
        SkyXploreLobbyPageActions.startGameCreation(driver1);
        SkyXploreLobbyPageActions.startGameWithMissingPlayers(driver1);
