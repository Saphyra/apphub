...
public class LinkActions {
    public static UUID createLink(int serverPort, String accessToken, CreateLinkRequest request) {
        Response response = getCreateLinkResponse(serverPort, accessToken, request);

        assertThat(response.getStatusCode()).isEqualTo(200);
        return response.getBody().jsonPath().getUUID("value");
    }

    public static Response getCreateLinkResponse(int serverPort, String accessToken, CreateLinkRequest request) {
