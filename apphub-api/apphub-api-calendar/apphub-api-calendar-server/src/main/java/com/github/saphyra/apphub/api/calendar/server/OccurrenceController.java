...
public interface OccurrenceController {
    ...
    void createOccurrence(@RequestBody OccurrenceRequest request, @RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessTokenHeader accessTokenHeader);

    @GetMapping(CalendarEndpoints.CALENDAR_GET_OCCURRENCES)
    List<OccurrenceResponse> getOccurrences(@RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessTokenHeader accessTokenHeader);
