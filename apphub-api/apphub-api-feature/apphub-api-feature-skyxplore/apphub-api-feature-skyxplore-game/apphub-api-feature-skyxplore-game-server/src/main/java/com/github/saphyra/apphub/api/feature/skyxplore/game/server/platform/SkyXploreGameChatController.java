...
public interface SkyXploreGameChatController {
    ...
    List<SkyXploreCharacterModel> getPlayers(@RequestParam(value = "excludeSelf", required = false, defaultValue = "false") Boolean excludeSelf, @RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessToken accessToken);

    @PutMapping(SkyXploreGameEndpoints.SKYXPLORE_GAME_CREATE_CHAT_ROOM)
    void createChatRoom(@RequestBody CreateChatRoomRequest request, @RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessToken accessToken);

    @DeleteMapping(SkyXploreGameEndpoints.SKYXPLORE_GAME_LEAVE_CHAT_ROOM)
    void leaveChatRoom(@PathVariable("roomId") String roomId, @RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessToken accessToken);

    @GetMapping(SkyXploreGameEndpoints.SKYXPLORE_GAME_GET_CHAT_ROOMS)
    List<ChatRoomResponse> getChatRooms(@RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessToken accessToken);
}
