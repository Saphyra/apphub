...
public interface MonitoringController {
    ...
    List<Feature> getFeatures(@RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessToken accessToken);

    @GetMapping(MonitoringEndpoints.MONITORING_GET_FUNCTIONALITIES)
    List<String> getFunctionalities(@PathVariable("feature") Feature feature, @RequestHeader(Constants.ACCESS_TOKEN_HEADER) AccessToken accessToken);
